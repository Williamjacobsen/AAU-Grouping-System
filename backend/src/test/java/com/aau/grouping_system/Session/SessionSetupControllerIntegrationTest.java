...
class SessionSetupControllerIntegrationTest {
	...
	void testEmailNewPasswordsToStudents_ValidRequest_ReturnsOk() throws Exception {
		...
		SessionSetupController.emailNewPasswordsRecord emailRecord = new SessionSetupController.emailNewPasswordsRecord(
				true);
	...
	void testEmailNewPasswordsToSupervisors_InvalidSessionId_ReturnsNotFound() throws Exception {
		...
		mockMvc
				.perform(
						post("/api/sessionSetup/{sessionId}/emailNewPasswordTo/supervisors", "session<script>alert('xss')</script>")
								.contentType(MediaType.APPLICATION_JSON)
								.content(requestJson))
				.andExpect(status().isNotFound());
	}
